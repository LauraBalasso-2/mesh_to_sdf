...
class SurfacePointCloud:
    ...
    def get_voxels(self, voxel_resolution, use_depth_buffer=False, sample_count=11, pad=False, check_result=False,
                   return_gradients=False):
        result = self.get_sdf_in_batches(get_raster_points(voxel_resolution), use_depth_buffer, sample_count,
                                         return_gradients=return_gradients)
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_centers=False, use_scans=True, sign_method='normal',
                                normal_sample_count=11, min_size=0, return_gradients=False, n_perturbations=2):
        query_points = []
        if use_centers:
            surface_sample_count = self.points.shape[0]
            surface_points = self.points
            unit_sphere_sample_count = int(0.07 * surface_sample_count)
        else:
...
def create_from_mesh(mesh, calculate_normals=True, cell_center=True):
    if cell_center:
    ...
    return SurfacePointCloud(mesh,
                             points=points,
                             normals=mesh.face_normals if calculate_normals else None,
                             scans=None)
